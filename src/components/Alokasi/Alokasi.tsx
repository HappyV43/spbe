...
import { normalizeDateFrom, normalizeDateTo } from "@/utils/color";
...
>({
  ...
  const chartAllocatedMonth = data
    .filter(
      (item) =>
        item.updatedAt >= currentMonthStart && item.updatedAt <= currentMonthEnd
    )
    .map((item) => ({
  ...
  const chartAllocatedToday = data
    .filter(
      (item) =>
        item.updatedAt >= normalizeDateFrom(today) &&
        item.updatedAt <= normalizeDateTo(today)
    )
    .map((item) => ({
      agentName: item.agentName,
      qty: item.allocatedQty,
  ...
  const handleSearch = () => {
    ...
    const filtered = data.filter((item) => {
      ...
      setFiltered(true);
      return (
        matchesStatus && matchesAgentName && matchesDate && matchesDoNumber
      );
    });
