...
export const calculateSummaryQty = (
  items: any[],
  propertyPath: string
): number => {
  return items.reduce((sum, item) => {
    ...
    const value =
      propertyPath.split(".").reduce((acc, key) => acc && acc[key], item) ?? 0;
...
export const calculateMontlyQty = (items: any) => {
  return items.reduce(
    (sum: any, item: { totalElpiji: any }) => sum + item.totalElpiji,
    0
  );
};
...
export const formatDateTime = (date: any) => {
  return date ? format(date, " PPPppp", { locale: id }) : "-";
};
